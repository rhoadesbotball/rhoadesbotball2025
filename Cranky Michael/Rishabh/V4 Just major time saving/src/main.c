...
{	
    ...
    freeze(1);
    msleep(2000);//wait for other robot
    ...
    msleep(200);
    drive(200,40);
    while(gmpc(1)>11000)//open claw partially to score 15 pts.
    {
        motor(1,-100);
    }
    freeze(1);
    thread motor_thread;
    motor_thread = thread_create(open);
    ...
    while(analog(0)>thresh0 || analog(1)>thresh1)// backwards square up with fail safe
    {
        motor(0,-70);
        motor(3,-69);
        if(analog(0)<thresh0 && analog(1)>thresh1)
    ...
    freeze(0);
    freeze(3);
    thread_wait(motor_thread);
    thread_destroy(motor_thread);
    while(analog(0)<thresh0 || analog(1)<thresh1)// square up
    {
        motor(0,70);
        motor(3,69);
