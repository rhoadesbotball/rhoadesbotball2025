...
void pvtlinefollow_fwd(int side, int speed, int timesTouchesLine)
{
    if(side==r)
    {
        for(int i=0;i<timesTouchesLine;i++)
        {
            while (analog(0)>thresh0)
            {
                mecwsleep(speed,0,0,speed);
            }
            while (analog(0)<thresh0)
            {
                mecwsleep(0,speed,speed,0);
            }
        }
    }
    if(side==l)
...
void gyroalpha_fwd(int speed, int cm)
{
...
void align_gyro_alpha()
{
    ...
    printf("hi\n");
    int speed=20;
    int bias12=70;  //speed*1.69801980198;//needs fix
    int bias03=45;  //speed*1.68965517241;//needs fix
